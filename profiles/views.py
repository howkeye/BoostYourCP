...
from .models import CodechefContest,Profile,CodeforceContest,Post,Announcements,chefuser,forceuser
import datetime
...
def contests(request):
    ...
    cdate = []
    for c in con:
        x=datetime.datetime.fromtimestamp(int(c.start))
        cdate.append(x)

    return render(request,'profiles/contest.html',{'codechef':CodechefContest.objects.all(),
                                                    'codeforces':CodeforceContest.objects.all(),'cdate' : cdate})
...
def getuser(request):
    ...
    for user in users:
        ...
        if(data['status']=='OK'):
            new_user.username=user
            new_user.currentrating=data['result'][0]['newRating']
            x=data['result'][0]['newRating']
            for i in data['result']:
                x=max(x,i['oldRating'])
            new_user.highestrating=x
            new_user.save()
    return redirect('/')


def ratings(request):
    context={ 'chefusers': chefuser.objects.all(),
             'forceuser' : forceuser.objects.all()
    }
